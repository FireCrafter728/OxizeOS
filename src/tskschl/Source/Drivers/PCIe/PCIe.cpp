...
{
    if(!mcfg) {
        printf("[TSKSCHL] [PCIe] [ERROR]: Invalid Initializer args\r\n");
        return false;
    }
...
bool PCIe::GetDeviceInfo(uint8_t bus, uint8_t device, DeviceInfo* infoOut)
{
    if(!infoOut || bus >= this->BussesImplemented || device >= PCIE_MAX_DEVICES) {
        printf("[TSKSCHL] [PCIe] [ERROR]: Invalid args passed to GetDeviceInfo()\r\n");
        return false;
    }
    ...
    if(!config) {
        printf("[TSKSCHL] [PCIe] [ERROR]: Memory allocation failed\r\n");
        return false;
    }
    ...
    for(size_t i = 0; i < deviceBarCount; i++)
    {
        ...
        if(!GetBARInfo(infoOut, &infoOut->BARs[i])) {
            printf("[TSKSCHL] [PCIe] [ERROR]: Failed to get BAR Info\r\n");
            return false;
        }
...
{
    if(!infoOut || !filters) {
        printf("[TSKSCHL] [PCIe] [ERROR]: Invalid args passed to LocateDevice()\r\n");
        return false;
    }
...
bool PCIe::GetBARInfo(DeviceInfo* device, BARInfo* info, uint8_t function)
{
    if(!device || !info || info->BarOffset < PCIE_BAR0 || info->BarOffset > PCIE_BAR5) {
        printf("[TSKSCHL] [PCIe] [ERROR]: Invalid args passed to GetBARInfo()\r\n");
        return false;
    }

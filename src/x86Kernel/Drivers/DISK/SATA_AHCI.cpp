...
{
    uint16_t command = PCI::ReadConfigWord(&device->info, 0x04);

    command |= (1 << 1) | (1 << 2);

    PCI::WriteConfigWord(&device->info, 0x04, command);
}

void MapAHCI_BAR5(SATA_AHCI::Device *device)
{
    device->AHCI_BAR = (volatile uint32_t *)PCI::ReadBAR(&device->info, PCI_BAR5_OFFSET);
    printf("[SATA AHCI] [INFO]: AHCI MMIO Base address: 0x%lx\r\n", (unsigned long)device->AHCI_BAR);
}

bool ResetController(SATA_AHCI::Device *device)
{
...
bool EnableAHCIMode(SATA_AHCI::Device *device)
{
    volatile uint32_t *ghc = &device->AHCI_BAR[SATA_AHCI_GHC_OFFSET / 4];
...
void InitializePorts(SATA_AHCI::Device *device)
{
    ...
    for (uint8_t i = 0; i < ImplementedPorts && i < SATA_AHCI_USED_PORT_COUNT; i++)
    {
        ...
        device->AHCIPorts[port].commandHeaders = (SATA_AHCI::CommandHeader *)(uintptr_t)(CommandListsAddr);
        ...
        {
            device->AHCIPorts[port].commandTables[slot] = (SATA_AHCI::CommandTable *)TableAddr;

            SATA_AHCI::CommandHeader *header = &device->AHCIPorts[port].commandHeaders[slot];
...
bool Identify(SATA_AHCI::Device *device, void *dataOut)
{
    SATA_AHCI::Port *port = &device->AHCIPorts[0];

    port->port_mmio[SATA_AHCI_PxIS / 4] = 0xFFFFFFFF;
    SATA_AHCI::CommandHeader *cmdHeader = &port->commandHeaders[SATA_AHCI_CMD_IDENTIFY];
    cmdHeader->CommandFISLength = 5;
    cmdHeader->WriteDirection = 0;
    cmdHeader->PRDTLength = 1;

    SATA_AHCI::CommandTable *cmdTable = port->commandTables[SATA_AHCI_CMD_IDENTIFY];
    ...
    SATA_AHCI::COMMAND_FIS *fis = (SATA_AHCI::COMMAND_FIS *)&cmdTable->CommandFIS[0];
    fis->fisType = 0x27;
...
bool SATA_AHCI::Initialize(SATA_AHCI::Device *device)
{
    ...
    EnableAHCI(device);
    MapAHCI_BAR5(device);
    if (!ResetController(device))
        return false;
    if (!EnableAHCIMode(device))
        return false;
    InitializePorts(device);

    if (!Identify(device, &device->IdentifyData))
        return false;
...
bool SATA_AHCI::ReadSectors(SATA_AHCI::Device *device, uint32_t lba, uint16_t count, void *dataOut)
{
    SATA_AHCI::Port *port = &device->AHCIPorts[0];
    port->port_mmio[SATA_AHCI_PxIS / 4] = 0xFFFFFFFF;

    SATA_AHCI::CommandHeader *cmdHeader = &port->commandHeaders[SATA_AHCI_CMD_READ];
    cmdHeader->CommandFISLength = 5;
    cmdHeader->WriteDirection = 0;
    cmdHeader->PRDTLength = 1;

    SATA_AHCI::CommandTable *cmdTable = port->commandTables[SATA_AHCI_CMD_READ];
    cmdTable->PRDTEntries[0].DataBaseAddressLow = (uint32_t)(uintptr_t)dataOut;
    cmdTable->PRDTEntries[0].DataBaseAddressHigh = 0;

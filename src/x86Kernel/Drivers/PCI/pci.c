...
static inline uint32_t PCI_MakeAddr(uint8_t bus, uint8_t device, uint8_t function, uint8_t offset)
{
...
{
    uint32_t address = PCI_MakeAddr(bus, device, function, 0);
    ...
    address = PCI_MakeAddr(bus, device, function, 0x08);
    outd(PCI_CONF_ADDR_PORT, address);
...
{
    uint32_t Address = PCI_MakeAddr(info->bus, info->device, info->function, offset & 0xFC);
    outd(PCI_CONF_ADDR_PORT, Address);
    uint32_t data = ind(PCI_CONF_DATA_PORT);
...
uint32_t PCI_ReadBAR(PCI_DeviceInfo* info, uint8_t BarOffset)
{
    ...
    address = PCI_MakeAddr(info->bus, info->device, info->function, BarOffset & 0xFC);

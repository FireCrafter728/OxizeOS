...
static void iTSCCalibrationEvent(SysKrnl64::ISR::Registers*);

bool sleepDone = false;
static void SleepEvent(SysKrnl64::ISR::Registers*);
...
bool Timer::Initialize(const TimerDesc* desc)
{
    ...
    if(!iTSC::isSupported())
    {
        ...
        primaryTimer = PrimaryTimer::HPET;
        return true;
    }

    primaryTimer = PrimaryTimer::iTSC;
    ...
    if(primaryTimer == PrimaryTimer::iTSC)
    {
    ...
    CPUID::CPUID_Regs maxLeaf = CPUID::GetCPUIDInfo(0x0);
    if(maxLeaf.rax >= 0x15)
    {
        leafExists = true;
        // Leaf supported, check if the fields are populated
        CPUID::CPUID_Regs iTSCLeaf = CPUID::GetCPUIDInfo(0x15);
...
void Timer::HPET_InterruptHandler(ISR::Registers* regs)
{
...
{
    if(nanoseconds < SLEEP_BUSYWAIT_TRESHOLD)
    {

...
MemoryAllocErrors HeapAlloc::Initialize(Heap_HeapAllocDesc* desc)
{
	...
	Heap_AllocationHeader* allocHdr = reinterpret_cast<Heap_AllocationHeader*>(heapVirtBase);
	allocHdr->flags = 0;
	allocHdr->allocSize = INITIAL_HEAP_SIZE - sizeof(Heap_AllocationHeader);
...
std::expected<void*, MemoryAllocErrors> HeapAlloc::AllocateBytes(size_t size)
{
	...
	Heap_AllocationHeader* hdr = reinterpret_cast<Heap_AllocationHeader*>(heapVirtBase);
	...
	if(origHdr.allocSize - size == 0) return reinterpret_cast<void*>(reinterpret_cast<uintptr_t>(hdr) + sizeof(Heap_AllocationHeader));

	// Free space left afterwards
	Heap_AllocationHeader* nextFreeHdr = reinterpret_cast<Heap_AllocationHeader*>(reinterpret_cast<uintptr_t>(hdr) + sizeof(Heap_AllocationHeader) + hdr->allocSize);
	nextFreeHdr->flags = 0;
	nextFreeHdr->allocSize = origHdr.allocSize - hdr->allocSize - sizeof(Heap_AllocationHeader);
...
MemoryAllocErrors HeapAlloc::FreeBytes(void* ptr)
{
	...
	if(next && !(next->flags & HEAP_FLAG_USED))
	{
		...
		header->allocSize += next->allocSize + sizeof(Heap_AllocationHeader);
	...
	if(prev && !(prev->flags & HEAP_FLAG_USED))
	{
		...
		prev->allocSize += header->allocSize + sizeof(Heap_AllocationHeader);
...
MemoryAllocErrors HeapAlloc::ExpandHeap()
{
	...
	hdr->flags = 0;
	hdr->allocSize = heapBlocks * BLOCK_SIZE - sizeof(Heap_AllocationHeader);
	hdr->next = nullptr;
	hdr->prev = lastHeader;
	lastHeader->next = hdr;
	lastHeader = hdr;
...
{
	auto res = heapAlloc->AllocateBytes(size);

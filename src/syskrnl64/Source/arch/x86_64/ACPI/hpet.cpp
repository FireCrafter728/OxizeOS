...
bool HPET_Timer::Initialize(HPET_Device* device)
{
	...
	device->regs = reinterpret_cast<volatile HPET_Registers*>(regAllocRes.value());
...
bool HPET_Timer::SetupTimer(HPET_Device* device, HPET_TimerDevice* timerOut, uint8_t* isrOut, bool periodic)
{
	...
	for(uint8_t gsi = 0; gsi < 32; gsi++)
	{
		...
		{
			isr = device->apic->AllocateSpecificGSI(gsi, IOAPIC_TRIGGER_EDGE | IOAPIC_TRIGGER_HIGH);
...
bool HPET_Timer::ArmTimer(HPET_Device* device, HPET_TimerDevice* timer, uint64_t targetNs, uint64_t counterBase)
{
...
bool HPET_Timer::StopTimer(HPET_Device* device, HPET_TimerDevice* timer)
{
	if(!device || !device->regs || !timer)
	{
...
uint64_t HPET_Timer::GetCounterTicks(HPET_Device* device)
{
	return device->regs->mainCounterValue;
}

uint64_t HPET_Timer::GetCounterFrequency(HPET_Device* device)
{
	return device->frequency;
}

bool HPET_Timer::ResetMainCounter(HPET_Device* device)
{
	if(!device)
	{
...
bool HPET_Timer::StartMainCounter(HPET_Device* device)
{
	if(!device)
	{
...
bool HPET_Timer::StopMainCounter(HPET_Device* device)
{
	if(!device)
	{
...
bool HPET_Timer::RestartMainCounter(HPET_Device* device)
{
	if(!device)
	{
...
uint64_t HPET_Timer::GetNSSinceStartup(HPET_Device* device)
{
	if(!device)
	{

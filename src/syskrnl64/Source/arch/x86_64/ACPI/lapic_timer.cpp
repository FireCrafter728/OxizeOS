...
bool LAPIC_Timer::Initialize(APIC* apic, ISR* isr, LPData* lpdata)
{
	if(!apic || !isr || !lpdata)
	{
	...
	cpuDescs.init(lpdata->GetLPCount(), defDesc);
...
bool LAPIC_Timer::InitCPU(LPID cpuID)
{
	...
	CPUID_Regs regs = GetCPUIDInfo(0x01);
...
bool LAPIC_Timer::Calibrate(LPID cpuID, uint64_t frequency)
{
	if(cpuID >= cpuDescs.size())
	{
...
bool LAPIC_Timer::SetMode(LPID cpuID, LT_Mode mode)
{
	if(cpuID >= cpuDescs.size())
	{
...
bool LAPIC_Timer::SetIntervalNS(LPID cpuID, uint64_t ns)
{
	if(cpuID >= cpuDescs.size())
	{
...
bool LAPIC_Timer::SetEvent(LPID cpuID, TimerEventHandler handler)
{
	if(cpuID >= cpuDescs.size())
	{
...
uint32_t LAPIC_Timer::GetCounterValue(LPID cpuID)
{
	if(cpuID >= cpuDescs.size())
	{
...
bool LAPIC_Timer::Start(LPID cpuID, uint64_t initialNS)
{
	if(cpuID >= cpuDescs.size())
	{
...
bool LAPIC_Timer::Stop(LPID cpuID)
{
	if(cpuID >= cpuDescs.size())
	{
...
void LAPIC_Timer::InterruptHandler(ISR_InterruptStackFrame* regs)
{
	// Get current LP LT CPU Desc
	LPSpecificData* lpData = LAPIC_Timer::instance->lpdata->GetLPDataForCurrentLP();

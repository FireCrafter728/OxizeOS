...
int main(int argc, char** argv)
{
	...
	while(std::getline(configFile, line))
	{
		...
		if(Command.value == "DISK_IMAGE") {
			...
			for(CfgArg& arg : args)
			{
				...
				else if(arg.key == "start") {
					op.args[4] = arg.value;
					partOffset.partIndex = std::stoul(op.args[6]);
					partOffset.partStart = arg.value;
			...
			{
				if(arg.key == "part") {
					index = std::stoul(arg.value);

					for(PartOffset offset : partOffsets)
					{
						if(offset.partIndex == index)
						op.args[4] = "--offset=" + offset.partStart;
					}
			...
			op.printfString = "\x1b[1m\x1b[33m[IMAGE]      \x1b[96mFormatting partition at index: " + std::to_string(index) + " with FAT32 and label: \"" + op.args[3] + "\"\x1b[0m\n"; 
			...
			for(CfgArg& arg : args)
			{
				if(arg.key == "path") {
					...
					op.args[1] = std::to_string(ltr->partIndex);

					op.args[3] = arg.value.substr(2);
				} else {
			...
			op.printfString = "\x1b[1m\x1b[33m[IMAGE]      \x1b[96mCreating directory with path: \"" + driveLetter + op.args[3] + "\"\x1b[0m\n";
			...
			for(CfgArg& arg : args)
			{
				if(arg.key == "src") op.args[3] = arg.value;
				else if(arg.key == "dst") {
					if(!isupper(static_cast<unsigned char>(arg.value[0])) || arg.value[1] != ':' || (arg.value[2] != '/' && arg.value[2] != '\\')) {
			...
			op.printfString = "\x1b[1m\x1b[33m[IMAGE]      \x1b[96mCopying file from host at path: \"" + op.args[3] + "\" to disk at path: \"" + driveLetter + op.args[4] + "\"\x1b[0m\n";
	...
	for(Operation& op : ops)
	{
		...
		switch(op.opType)
		{
			...
			case MKDIR:
			case COPY: {
				operation = "output/fat32";
				break;
			}
			case MKFS: {
				operation = "/usr/bin/mkfs.fat";
				break;
			}
		}

...
int main(int argc, char **argv)
{
    ...
    FAT32::MBR::MBR *mbr = new FAT32::MBR::MBR;
    if (!mbr)
    {
        fprintf(stderr, "[FAT32-MAIN] [ERROR]: Failed to allocate memory for MBR\n");
        delete (parser);
        delete (disk);
        return 5;
    }

    FAT32::MBR::MBRDesc mbrDesc = {};

    int partition = -1;
    if (args.PartitionIndex != 0)
        partition = args.PartitionIndex;
    if (!mbr->Initialize(disk, &mbrDesc, partition))
    {
        fprintf(stderr, "[FAT32-MAIN] [ERROR]: Failed to initialize MBR\n");
        delete (parser);
        delete (disk);
        delete (mbr);
        return 6;
    ...
    case FAT32::Parser::Operations::ListContents:
    {
        ...
        while (fat->ReadEntry(&current, &Volume) && !(Volume.SFNEntry.Attribs & FAT32::FAT::FileAttribs::VOLUMEID))
            ;
        printf("Volume in partition %d is %*s\n", mbr->GetPartitionIndex(), 11, Volume.SFNEntry.Name);

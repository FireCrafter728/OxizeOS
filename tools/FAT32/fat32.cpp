...
{
    return mbr->ReadSectors(0, 1, &data->BS.bootSectorBytes);
}

bool FAT::ReadFAT(m_uint32_t lbaIndex)
{
    return mbr->ReadSectors(fatData.FatStartLba + lbaIndex, FAT_CACHE_SIZE, data->FatCache);
}

bool FAT::Initialize(MBR::MBR *mbr)
{
    this->mbr = mbr;
...
void FAT::InitializeMin(MBR::MBR *mbr)
{
    this->mbr = mbr;
}
...
{
    m_uint32_t totalSectors = mbr->GetPartitionSectorCount();
    ...
    bootSector.bootSig = MBR_BOOT_SIGNATURE;
    ...
    bootSector.bpb.HiddenSectors = mbr->GetPartitionStartLba();
    ...
    bootSector.ebr.SectorsPerFAT = (TotalClusters * 4 + desc->BytesPerSector - 1) / desc->BytesPerSector;
    ...
    if (!mbr->WriteSectors(0, 1, &bootSector))
    {
        fprintf(stderr, "[FAT32] [ERROR]: Failed to write to DISK\n");
        return false;
    }

    if (!mbr->WriteSectors(bootSector.ebr.backupBootSector, 1, &bootSector)) // backup boot sector
    {
        fprintf(stderr, "[FAT32] [ERROR]: Failed to write to DISK\n");
        return false;
    }
    ...
    if (!mbr->WriteSectors(bootSector.ebr.FSInfoSector, 1, &fsinfo))
    {
        fprintf(stderr, "[FAT32] [ERROR]: Failed to write to DISK\n");
        return false;
    }
    ...
    {
        if (!mbr->WriteSectors(bootSector.bpb.ReservedSectors + i, 1, &buffer))
        {
            fprintf(stderr, "[FAT32] [ERROR]: Failed to write to DISK\n");
            return false;
        }
    ...
    if (!mbr->WriteSectors(bootSector.bpb.ReservedSectors, 1, &buffer))
    {
        fprintf(stderr, "[FAT32] [ERROR]: Failed to write to DISK\n");
        return false;
    }

    if (!mbr->WriteSectors(bootSector.bpb.ReservedSectors + bootSector.ebr.SectorsPerFAT, 1, &buffer))
    {
        fprintf(stderr, "[FAT32] [ERROR]: Failed to write to DISK\n");
        return false;
    }
...
bool FAT::WriteFATEntry(m_uint32_t cluster, m_uint32_t value)
{
    ...
    *(m_uint32_t *)&data->FatCache[offsetInCache] = value & 0x0FFFFFFF;
    ...
    for (m_uint32_t i = 0; i < bpb->FATCount; i++)
    {
        ...
        if (!mbr->WriteSectors(fatSector, FAT_CACHE_SIZE, data->FatCache))
        {
            fprintf(stderr, "[FAT32] [ERROR]: Failed to write to DISK\n");
            return false;
        }
...
bool FAT::ReadEntry(File *dir, LFNDirectoryEntry *entryOut)
{
    ...
    *entryOut = {};
    ...
    while (true)
    {
        ...
        if (!mbr->ReadSectors(lba, 1, buffer))
        {
...
m_uint32_t FAT::ReadFile(File *file, void *buffer, m_uint32_t size)
{
    ...
    while (bytesLeft > 0 && file->CurrentCluster != 0xFFFFFFFF && file->Position < file->Size)
    {
        ...
        if (!mbr->ReadSectors(lba, 1, sector))
        {
...
bool FAT::CreateDirectoryEntry(File *dir, LPCSTR name, LFNDirectoryEntry *entry)
{
    ...
    if (entry->SFNEntry.Attribs & FileAttribs::DIRECTORY)
    {
        ...
        if (!WriteFATEntry(newDirCluster, 0x0FFFFFFF))
        {
            fprintf(stderr, "[FAT32] [ERROR]: Failed to write FAT entry for new directory cluster\n");
            return false;
        }

        ZeroCluster(newDirCluster);
            ...
            for (m_uint32_t sectorOffset = 0; sectorOffset < bpb->SectorsPerCluster; sectorOffset++)
            {
                ...
                for (size_t i = 0; i < entriesPerSector; i++)
                {
                    if (entries[i].Name[0] == 0x00 || entries[i].Name[0] == 0xE5)
                    {
                        ...
                        if (!mbr->WriteSectors(lba, 1, buffer))
                        {
                            fprintf(stderr, "[FAT32] [ERROR]: Failed to write to DISK\n");
                            return false;
                        }
...
void FAT::ZeroCluster(m_uint32_t cluster)
{
    ...
    {
        if (!mbr->WriteSectors(startLba + sector, 1, buffer))
            fprintf(stderr, "[FAT32] [ERROR]: Failed to write to DISK\n");
    }
}
...
bool FAT::CopyEntry(LPCSTR path, LPCSTR newPath, bool isDirectory, File *newFile)
{
    ...
    if (isDirectory)
    {
        ...
        {
            if (!mbr->ReadSectors(ClusterToLba(srcCluster), bpb->SectorsPerCluster, &buffer))
            {
                fprintf(stderr, "[FAT32] [ERROR]: Failed to read from DISK\n");
                return false;
            }

            if (!mbr->WriteSectors(ClusterToLba(destCluster), bpb->SectorsPerCluster, &buffer))
            {
                fprintf(stderr, "[FAT32] [ERROR]: Failed to write to DISK\n");
                return false;
            }
...
bool FAT::SetFileData(LPCSTR filePath, void *data, m_uint32_t count)
{
    ...
    for (m_uint32_t i = 0; i < clustersNeeded; i++)
    {
        ...
        if (!mbr->WriteSectors(ClusterToLba(currentCluster), bpb->SectorsPerCluster, dataPtr))
        {
            fprintf(stderr, "[FAT32] [ERROR]: Failed to write  to DISK\n");
            return false;
        }
